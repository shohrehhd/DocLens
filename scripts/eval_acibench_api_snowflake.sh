...
DEFAULT_SAVENAME=acibench-test1${TAG}-${MODEL}-shot${SHOT}-quick_test3 # remove -quick_test3 if no quick test
...
python claim_evaluation/generate_subclaims.py --eval_file data/ACI-Bench-TestSet-1_clean.json \
    ...
    --prompt_file claim_evaluation/prompts/acibench_persection_subclaim_generation.json --provider snowflake
...
python claim_evaluation/run_entailment.py --result_file results/${SAVENAME}.claim_min1max30.json \
    ...
    --prompt_file claim_evaluation/prompts/acibench_claim_entail.json --provider snowflake
...
python citation_evaluation/eval_citation.py --result_file results/${SAVENAME}.json \
    ...
    --prompt_file citation_evaluation/prompts/acibench_persection_citation_entail.json --provider snowflake
